...
| Agent 配置 | `meeting-agent-prompts-v12` · `meeting-agent-state-v7` |
...
| 真实模型完整场景 | 120 | **119/120 · 99.17%** | 意图、约束、工具计划与回答质量 |
| 重复运行稳定性 | 90 | **89/90 · 98.89%** | 30 条核心场景分别运行 3 次，稳定 Case 为 29/30 |
| 隔离端到端轨迹 | 8 | **8/8 · 100%** | 创建、改期、取消与 HITL 副作用 |
| 公共 API 多轮场景 | 16 | **16/16 · 100%** | API、Agent、状态恢复与业务终态 |
...
| 运行范围 | P50 | P95 |
| --- | ---: | ---: |
| 真实模型完整场景 | **3.09s** | **4.32s** |
| 隔离端到端轨迹 | 7.01s | 9.33s |
| 公共 API 多轮场景 | 9.03s | 14.61s |
