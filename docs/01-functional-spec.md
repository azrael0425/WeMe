...
- 查看会议详情和参与者。
- 手动创建、修改、取消。
- 手动创建和修改时由用户输入参会人姓名；前端仅用安全在职员工目录进行精确姓名解析，并将解析出的员工 ID 写入既有参会人数组。重名必须补充“姓名（部门）”，解析失败或仍不唯一时禁止提交。
...
| AC-35 | 账号退出或切换后本地会话缓存被安全清除；重新登录原账号时，最近任务、完整用户可见消息和多个待确认草案可从服务端恢复，其他账号不可见 |
| AC-36 | RAG 首次查询不再无限承担模型冷启动：启动预热、周期保温和缓存生效，Embedding 超过 8 秒时使用仍带真实引用的有界关键词降级 |
| AC-37 | Scheduling 在事实齐备后立即停止模型循环，最多 4 轮；DeepSeek 连接复用且默认单次超时/重试有界，SSE 首事件不等待完整推理 |
