...
1. 发起人为未来 `CONFIRMED` 会议保存议程和材料元数据，参会者写请求被拒绝，过期 `preparationVersion` 不能覆盖新内容。
2. 断言准备清单随议程、材料、会议室和参与者当前事实变化；原子替换失败时旧数据完整保留。
3. 重复执行调度扫描，24 小时、30 分钟和缺失项通知各最多一份；会议结束后只发生一次 `CONFIRMED -> COMPLETED`。
4. 提交虚构文本会议记录，断言现有 Requirement Agent 返回结构化草案，非法负责人不成为业务 ID；Agent 失败时正式表为空。
5. 对草案执行 EDIT，断言仍无正式数据且版本递增；再 ACCEPT 后纪要、决策和行动项在同一事务出现。REJECT 与重复审核无副作用。
6. 行动项负责人更新状态；重复临期/逾期扫描不重复通知，`DONE` 项不再催办。
7. 前端 `/lifecycle` 全程读取真实 `/api/v1/**`，不存在 RSVP、签到、附件、政策绑定或统计预览。
...
9. 会前准备和会后正式记录在容器重启后仍存在，静态 preview 数据不参与结果。
