...
Refero 页面用于研究真实产品的布局、信息层级和交互模式，不授权复制品牌、图片、Logo、字体文件或源代码。最终界面必须使用 WeMe 自有文字、业务数据和组件。
...
四套参考只提供布局模式，最终必须通过一套 WeMe Design Token 统一：
