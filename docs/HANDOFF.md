...
## 2026-08-15 最近任务限制在左栏上三分之一

- 结论：**PASS。** “最近任务”位于左侧栏上部，不再停靠底部。品牌区下方的“新建编排、搜索、最近任务”共同被限制在侧栏高度的上三分之一；任务列表本身在可用高度内滚动，不会挤占下方主导航或用户信息。
- 保留前一轮按 `threadId` 聚合和恢复同一对话历史的行为；本次没有变更 API、数据、Agent 或正式会议。
- 证据：前端 `npm run type-check`、`npm run build`、`docker compose config --quiet` 与 `git diff --check` 均 PASS；已重建健康的前端容器并刷新本地页面。未修改 `.env` 或命名卷。

## 2026-08-15 最近任务按对话线程聚合

- 结论：**PASS。** 根因是前端已用 `threadId` 保存会话历史，却在侧栏把 `meetops.chat-run-context.v1` 中每个 `runId` 平铺为一项，导致一次对话的每次独立提问都显示成最近任务。现在改为按 `threadId` 聚合：每个对话只显示一项，保留首个问题作为稳定标题，使用最新 Run 的状态和跳转目标，并使搜索覆盖该线程的所有提问。
- 进入最近任务会通过最新 Run 恢复同一线程已保存的用户问题和 Agent 回答；无需新增 API、数据库表或产品 Agent。历史继续按当前已登录浏览器会话存储，退出登录和清除会话会按既有隐私边界清除本地缓存。
- 证据：前端 `npm run type-check`、`npm run build`、容器内生产构建、`docker compose config --quiet` 与 `git diff --check` 均 PASS；前端已重建，Agent、前端与 Java 服务均 healthy。本次未创建或修改正式会议、未修改 `.env` 或命名卷。
