...
## 2026-08-15 最近任务移至左侧栏底部

- 结论：**PASS。** “最近任务”已从主导航顶部移至左侧栏底部、用户信息上方。主导航仍独立滚动，最近任务最多占用 248px 并单独滚动，避免任务较多时挤压核心导航；折叠和移动端导航保持原有行为。
- 证据：前端 `npm run type-check`、`npm run build`、`docker compose config --quiet` 与 `git diff --check` 均 PASS；已重建前端容器并重新加载本地页面，前端服务 healthy。本次未创建或修改正式会议、未修改 `.env` 或命名卷。
