<template>
  <AppShell title="异常重排" description="在资源失效时比较原计划与替代方案，保留人工决策边界。" eyebrow="产品预览 / 异常重排">
    ...
    <div class="preview-grid">
      <section class="content-panel preview-event"><span class="preview-icon">!</span><div><p>资源异常</p><h2>{{ replanPreview.event }}</h2><span>影响 {{ replanPreview.affected.length }} 场会议 · 需人工确认替代计划</span></div></section>
      <section class="content-panel"><h2>受影响会议</h2><ul class="clean-list"><li v-for="meeting in replanPreview.affected" :key="meeting"><span>▦</span><strong>{{ meeting }}</strong><StatusBadge status="CONFLICT" /></li></ul></section>
    </div>
    ...
    <section class="content-panel preview-section"><h2>约束变化与放宽原因</h2><dl class="preview-facts"><div><dt>约束变化</dt><dd>{{ replanPreview.constraintChange }}</dd></div><div><dt>放宽原因</dt><dd>{{ replanPreview.relaxationReason }}</dd></div></dl><h3>未受影响项</h3><div class="check-list"><span v-for="item in replanPreview.preserved" :key="item">✓ {{ item }}</span></div><button class="ui-button ui-button--default" type="button" @click="notice=true">应用替代计划</button><p v-if="notice" class="preview-notice" role="status">产品预览尚未连接后端，未执行任何写操作。</p></section>
  </AppShell>
</template>
